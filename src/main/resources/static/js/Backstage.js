        ...
        $.ajax({
            ...
            success:function (ret) {
                ...
                for (var i = 0 ;i<ret.length;i++) {
                    xvalue.push(ret[i].c_count);
                    yvalue.push(ret[i].c_name);
                }
                showcharts(xvalue,yvalue);
            }
        });
    })
});
...
// 修改用户信息  未完成
function updateUser() {
    $("tbody").on("click","a[name='修改用户']",function (event) {
        alert("修改用户");
